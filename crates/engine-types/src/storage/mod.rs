...
//! [`ActivityRecord`], [`TaskRecord`], [`TimerRecord`]). The concrete implementations — the durable
//! `RocksStorage`, the in-memory `InMemoryStorage`, and their byte-level `KeyBuilder` encoding — live
//! in `spica-storage`, which **depends on** this crate to implement the trait. Both sides therefore
//! meet on one trait type: the engine's consumers and the storage backends each name the same
//! [`Storage`], so an implementation satisfies the engine's bounds wherever it is compiled.
...
/// [`Event`](crate::types::event::Event) stream.
///
/// This is the **read** interface used by handlers and the cascade (they observe snapshots and never
/// mutate). The actual projection of events is performed by the engine's `dispatch_event`
/// implementations, which mutate this store through the `put_*` / `remove_child` methods below —
/// mirroring how command handlers observe Storage but delegate their output to the engine's
/// `Collector`.
...
/// `InMemoryStorage`); the trait sits here alongside [`Storage`] so both the engine and the storage
/// crate name one contract type rather than each defining its own.
