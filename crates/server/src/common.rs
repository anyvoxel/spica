...
/// [`Gateway`] application layer that owns the blocking request/response API.
///
...
/// [`Gateway`] wraps it and the shared [`AckHook`](crate::gateway::AckHook). Only `Engine::start` —
...
pub(crate) struct Svc {
    ...
    /// The application layer (blocking request/response API over `engine`) the request/response
    /// handlers forward to: `create_flow`, `start_for_revision`, and the task claim/settle API.
