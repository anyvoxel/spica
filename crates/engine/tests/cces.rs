...
use spica_engine::{
    ActivityId, Command, Engine, Entry, EntryPayload, Event, ExecutionError, ExecutionId,
    ExecutionStatus, IdSource, InMemoryLogStream, InMemoryStorage, LogStream, NodeId, Processor,
    Storage, TerminationReason, TimerId, TimerPurpose, Timestamp,
};
...
struct Projector {
    ...
    task_service: spica_engine::TaskServiceHandle,
}

impl Projector {
    fn new() -> Self {
        // Fire channel is unread here: projection tests only fold state, they never wait on a
        // fired timer / settled task. The loops exit once the handles drop below.
        let (fire_tx, _fire_rx) = tokio::sync::mpsc::unbounded_channel::<(
            spica_engine::StreamId,
            spica_engine::EntryId,
            Command,
        )>();
        let (scheduler, _handle) = spica_engine::SchedulerHandle::spawn(fire_tx.clone());
        let (task_service, _handle) =
            spica_engine::TaskServiceHandle::spawn(std::collections::HashMap::new(), fire_tx);
        Self {
...
async fn collect_events(sm: StateMachine, input: Value) -> Vec<Event> {
    ...
    // Scheduler + task service + their shared fire channel, wired the same way `Processor::run`
    // wires them.
    ...
    let (task_service, _handle) =
        spica_engine::TaskServiceHandle::spawn(std::collections::HashMap::new(), fire_tx);
    // Handles dropped at scope end are fine: the loops exit once `scheduler` is dropped below.
