//! The `Succeed` state's lifecycle, pinned as a whole **entry chain** rather than as a terminal status.
...
#[rustfmt::skip]
#[tokio::test]
async fn succeed_with_explicit_output() {
    let definition = r#"{
    "StartAt": "S",
    "States": { "S": { "Type": "Succeed", "Output": { "ok": true, "count": 7 } } }
}"#;
    let case = TypedCase {
        definition,
        input: r#"{"n":1}"#,
...
#[rustfmt::skip]
#[tokio::test]
async fn succeed_without_output_passes_its_input_through() {
    let definition = r#"{
    "StartAt": "S",
...
#[rustfmt::skip]
#[tokio::test]
async fn succeed_output_as_jsonata() {
    let definition = r#"{
    "StartAt": "Seed",
    "States": {
        "Seed": { "Type": "Pass", "Assign": { "code": "E-42" }, "Next": "S" },
        "S": { "Type": "Succeed", "Output": { "echo": "{% $states.input.n %}", "code": "{% $code %}" } }
    }
}"#;
    let case = TypedCase {
        definition,
