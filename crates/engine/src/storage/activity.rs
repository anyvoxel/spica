...
/// Lifecycle status of an [`Activity`] — the execution of a single state within an [`Execution`](super::Execution).
///
/// Mirrors [`super::ExecutionStatus`] exactly (`Running` -> `Completing` -> `Completed` success, and
...
/// status can't be confused for its execution's — a container `Activity` (Map/Parallel) is `Running`
/// while owning its children, then drains through `Completing`/`Terminating` just like the execution
/// does.
...
/// The execution of a single state within an [`Execution`](super::Execution). Owned by its
...
pub struct Activity {
    ...
    /// here. Carried on `Event::StateActivating` (see its docs) so a follower / recovered leader
    /// records exactly where the activity lives without re-deriving it from the machine + parent chain.
    ...
    /// kept verbatim, distinct from [`Activity::input`] (the value after the state's own input
    /// preprocessing, e.g. projecting `Arguments`), so a follower / auditor can inspect both the
    /// original and the processed view of what the state ran on.
    pub raw_input: Value,
    /// The input this state actually processes — the result of the state's dialogue-level input
    /// preprocessing applied to [`Activity::raw_input`]. For states that consume their raw input
    /// verbatim (Pass/Choice/Wait/Succeed/Fail and Map/Parallel activations without `Arguments`),
    /// this is a copy of `raw_input` (never `None`); for a `Task`/`Parallel` with `Arguments` it is
    /// the projected arguments (the value a `$states` projection would see). Kept so the processed
    ...
    /// Kept distinct from [`Activity::output`] so the engine preserves both the pre-projection and the
    ...
    /// grows by adding a variant for a new container state. See [`ActivityState`].
...
/// The **state-specific runtime repository** of an [`Activity`] — data only a container state
/// carries, kept as a typed enum instead of `Option`/empty-collection fields on the shared skeleton.
///
/// The shared lifecycle skeleton (`id`/`parent`/`state`/`status`/`raw_input`/`input`/`retry_state`/
/// `raw_output`/`output`/`active_children`/...) lives on [`Activity`] directly because every state needs
/// it; this enum holds only what *some* states need. A new container state (or a Map gaining
...
/// `Activity` already carries the shared lifecycle skeleton; this payload holds only the
...
/// replenish loop in [`MapStateHandler`](crate::handlers::states::map::MapStateHandler):
///
