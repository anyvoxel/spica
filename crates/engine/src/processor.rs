...
use crate::handlers::{
    ActivateStateHandler, ActivateTaskHandler, ActivateTimerHandler, CancelTaskHandler,
    CancelTimerHandler, CompleteExecutionHandler, CompleteStateHandler, CompleteTaskHandler,
    CompleteTimerHandler, CreateExecutionHandler, ProcessChildCompletedHandler, SpawnBranchHandler,
...
use crate::task_service::{TaskHandler, TaskServiceHandle};
...
impl Processor {
    pub fn new(sm: StateMachine, execution: ExecutionId) -> Self {
        Self::with_task_handlers(sm, execution, HashMap::new())
    }

    /// Build a `Processor` that dispatches external `Task` calls to the provided per-`Resource`
    /// handlers. `task_handlers` maps a `Resource` URI (the `Task` state's `resource`) to the
    /// handler that executes that call; a `Task` whose `resource` has no handler fails at
    /// invocation.
    pub fn with_task_handlers(
        sm: StateMachine,
        execution: ExecutionId,
        task_handlers: HashMap<String, Arc<dyn TaskHandler>>,
    ) -> Self {
    ...
    pub async fn run<L: LogStream, S: Storage>(
        ...
        // Channel over which the scheduler (fired timers) and task service (settled tasks) return
        // their resumption commands to this loop. Both are cloned feeds into one receiver — this
        // loop stays the single writer to the logstream.
        let (fire_tx, mut fire_rx) = mpsc::unbounded_channel::<(StreamId, EntryId, Command)>();
        let (_scheduler, _handle) = SchedulerHandle::spawn(fire_tx.clone());
        let (_task_service, _task_handle) =
            TaskServiceHandle::spawn(std::mem::take(&mut self.task_handlers), fire_tx);
...
pub(crate) fn log_event(event: &Event) {
    match event {
        ...
        Event::StateActivating {
            ...
            info!(
                activity = %activity,
                state = %crate::handlers::state_name_from_path(state_path.as_ptr()),
                input = %input,
                "entered state"
            );
        }
        ...
        Event::StateTerminating { activity, reason } => {
            warn!(activity = %activity, reason = %format!("{reason:?}"), "state terminating");
        }
        ...
        Event::TaskActivated {
            ...
            debug!(task = %task, parent = ?parent, resource = %resource, "task activated");
        }
        Event::TaskCompleted { task, output } => {
            debug!(task = %task, output = %output, "task completed");
        }
        Event::TaskFailed { task, error } => {
            warn!(task = %task, error = %format!("{error:?}"), "task failed");
        }
