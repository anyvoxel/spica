...
use crate::id::NodeId;
use crate::storage::{RetryState, TaskStatus};
...
/// Handles `CompleteTask`: a foreign `Task` settled. Idempotent (a no-op if the task is gone or
/// already terminal). Dispatches by the carried `result`:
///
/// - `Ok(output)` → the task succeeded: emit `TaskCompleted`, then resume the owning state by
///   throwing `CompleteState` (its `complete` runs the success projection and routes to `Next`/`End`).
/// - `Err(error)` → the task failed: emit `TaskFailed`, then route the failure through the state's
///   error-handling policy — first `Retry` (re-arm the task on a backoff timer), then `Catch`
///   (bind `$states.errorOutput` and route to the catcher's `Next`) — and only when neither matches
///   propagate up through the same termination cascade a `Fail` state uses.
#[derive(Default)]
...
impl CommandHandler for CompleteTaskHandler {
    fn command(&self) -> Command {
        Command::CompleteTask {
            ...
            result: Err(crate::error::ExecutionError::InvalidDefinition(
                String::new(),
            )),
        }
    }
    ...
    async fn handle(&self, cmd: &Command, ctx: &mut HandlerContext<'_>, out: &mut Collector) {
        let Command::CompleteTask { task, result } = cmd else {
            unreachable!(
        ...
        let act = match ctx.storage.get_task(*task).await {
            Ok(Some(t)) => t,
            Ok(None) | Err(_) => return, // task never invoked; nothing to do.
        };
        if act.status != TaskStatus::Active {
            return; // already completed/failed/cancelled — a duplicate settle is a no-op.
        }
        ...
        match result {
            Ok(output) => {
                ...
                // Resume the owning Task state's `complete`.
            ...
            Err(error) => {
                ...
                // backoff), then `Catch` (bind errorOutput + route to the catcher's Next), then
                // terminate. This keeps the policy local to the state definition and defers the
                // decision to the next dispatch round where a retry timer / catcher runs.
...
impl CompleteTaskHandler {
    ...
    async fn route_failure(
        ...
        let execution = match activity.parent {
        ...
        if let Some((retrier_index, retry)) = task_state.retry.as_deref().and_then(|rs| {
            ...
            let retrier_attempts = retrier_attempt_count(&activity.retry_state, retrier_index);
            ...
            if (retrier_attempts as i64) < max_attempts {
                ...
                let next_retry_count = activity.retry_state.retry_count + 1;
        ...
        if let Some(catcher) = task_state.catch.as_deref().and_then(|cs| {
            ...
            let exec = match activity.parent {
                NodeId::Execution(e) => e,
                _ => return,
            };
            ...
            let actx = ActivityCtx {
                execution: exec,
                root_execution: ex.root_execution,
                execution_state_path: ex.state_path.clone(),
                exec_input: ex.input.clone(),
                raw_input: activity.raw_input.clone(),
                input: activity.input.clone(),
                raw_output: activity.raw_output.clone(),
                scope: ex.scope.clone(),
                state_path: activity.state_path.clone(),
                retry_count: activity.retry_state.retry_count,
    ...
    async fn terminate_failure(
        ...
        let execution_id = match ctx.storage.get_activity(activity_id).await {
            Ok(Some(a)) => match a.parent {
                NodeId::Execution(e) => e,
...
mod tests {
    ...
    use crate::error::ExecutionError;
    use crate::storage::{RetrierAttemptState, RetryState};
