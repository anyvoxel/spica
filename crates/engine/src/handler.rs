...
use crate::id::{ActivityId, EntryId, ExecutionId, StreamId, TaskId, TimerId};
...
pub struct ActivityCtx {
    /// The execution that owns this activity. For a top-level (non-branch) activity this is the
    /// top-level run; for an activity inside a Parallel branch it is that branch's child execution.
    pub execution: ExecutionId,
    /// The id of the top-level run this activity belongs to (the flat query anchor). For a
    /// top-level activity this equals `execution`; inside a branch it is the branch's
    /// `root_execution` — carried so a `Parallel` fan-out can pass the true root to its children.
    ...
    /// The **raw** input this state received on entry — verbatim, before any activation-time input
    /// preprocessing. Mirrors [`crate::storage::Activity::raw_input`] and the value carried on
    /// [`crate::Event::StateActivating`].
    pub raw_input: Value,
    /// The input this state actually processes — the result of activation-time input preprocessing
    /// (e.g. projecting a `Task`/`Parallel` `Arguments`) applied to `raw_input`. For states that
    /// consume their raw input verbatim this is a copy (never `None`). Mirrors
    /// [`crate::storage::Activity::input`].
    pub input: Value,
    /// The state's raw result before any complete-step `Output` projection. Mirrors
    /// [`crate::storage::Activity::raw_output`].
    ...
    /// Parallel branch). The leaf state name (its key in the enclosing `states` table) is **derived**
    /// as the pointer's last token ([`ActivityCtx::state_name`]), so only the full path is carried —
    /// state identity and location are one value, not two that can drift.
    pub state_path: jsonptr::PointerBuf,
    /// Number of retry attempts already made for this activity (0 on first entry). Binds
    /// `$states.context.State.RetryCount` and drives a `Retry`'s backoff/attempt policy.
    pub retry_count: u32,
    pub kind: CtxKind,
}
...
impl ActivityCtx {
    ...
    pub fn state_name(&self) -> String {
        crate::handlers::state_name_from_path(self.state_path.as_ptr())
    }
}
