...
impl Engine {
    ...
    pub async fn start(
        ...
        Self::start_with_task_handlers(sm, input, std::collections::HashMap::new()).await
    }

    /// Like [`Engine::start`], but dispatches external `Task` calls to per-`Resource` handlers —
    /// `task_handlers` maps each `Task` state's `Resource` URI to the handler that executes that
    /// call. A `Task` whose `Resource` has no registered handler fails at invocation.
    pub async fn start_with_task_handlers(
        sm: StateMachine,
        input: Value,
        task_handlers: std::collections::HashMap<
            String,
            std::sync::Arc<dyn crate::task_service::TaskHandler>,
        >,
    ) -> Result<crate::result::ExecutionResult, ExecutionError> {
        use std::sync::Arc;
        ...
        let mut processor = Processor::with_task_handlers(sm, execution_id, task_handlers);
